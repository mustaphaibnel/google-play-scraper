...
function initialRequest (opts) {
  ...
  function skipClusterPage (html) {
    const match = html.match(/href="\/work\/apps\/collection\/search_collection_more_results_cluster?(.*?)"/);
    if (match) {
  ...
  const url = `${BASE_URL}/work/search?c=apps&q=${opts.term}&hl=${opts.lang}&gl=${opts.country}&price=${opts.price}`;
